...
def train_models():
    ...
    def balance_dataset(X, y):
        ...
        for cls in unique_classes:
            ...
            X_resampled, y_resampled = resample(
                X_cls, y_cls, replace=replace, n_samples=500, random_state=42
            )
    ...
    knn_search = GridSearchCV(
        knn_pipeline, knn_params, cv=5, n_jobs=-1, verbose=2, scoring="accuracy"
    )
    ...
    svm_search = GridSearchCV(
        svm_pipeline, svm_params, cv=5, n_jobs=-1, verbose=2, scoring="accuracy"
    )
